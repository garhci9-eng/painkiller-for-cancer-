...
> **[KO]** 마약성(오피오이드) 의존 없이 심한 암성 통증에 효과적인 신약 후보를 AI로 탐색하는 웹 앱
>
> **[EN]** A web application that uses AI to discover non-opioid drug candidates effective against severe cancer pain — without relying on addictive substances.
...
**[KO]**
암성 통증(Cancer Pain)은 환자 삶의 질에 심각한 영향을 미칩니다. 현재 치료는 마약성 진통제(오피오이드)에 의존하는 경우가 많으나, 중독·부작용 문제가 큽니다. 이 프로젝트는 공개 임상 데이터(ChEMBL)를 활용해 마약성 의존 없이도 효과적인 신약 후보 화합물을 AI로 탐색합니다.

**[EN]**
Cancer pain severely affects patients' quality of life. Current treatments often rely on opioids, which carry significant risks of addiction and side effects. This project leverages open clinical data (ChEMBL) to discover effective non-opioid drug candidates using AI-powered screening.
...
### 방법 1 — Python 직접 실행 / Method 1 — Run with Python
...
└── data/
    └── compounds.db        # 자동 생성 / Auto-generated
```
...
| 데이터 수집 / Data Fetching | ChEMBL WebResource Client |
...
**[KO]** 이 프로젝트는 **사용자 아이디어 50% + Claude AI (Anthropic) 아이디어 50%** 로 공동 창작되었습니다.

**[EN]** This project was **co-created by the user (50%) and Claude AI by Anthropic (50%)**.
...
## ⚕️ 의료 고지 / Medical Disclaimer

**[KO]** 이 소프트웨어는 연구 참고용입니다. 실제 임상 또는 의약품 개발 적용 시 반드시 전문 의료·약학 전문가의 검토를 받으세요.
...
- [ChEMBL](https://www.ebi.ac.uk/chembl/) — 화합물-타겟 활성 데이터 / Compound-target activity data
- [PubChem](https://pubchem.ncbi.nlm.nih.gov/) — 분자 구조 데이터 / Molecular structure data
- [RDKit](https://www.rdkit.org/) — 화학정보학 라이브러리 / Cheminformatics library
